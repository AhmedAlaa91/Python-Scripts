...
server = 'mal-sqlsrv-wus2.database.windows.net'
database = 'AIW-PROD-DB'
username = 'maladmin'
password = 'JzNiP3x*Whd#nQ'
InitialCatalog='MALDB'
driver= '{ODBC Driver 17 for SQL Server}'
cnxn = pyodbc.connect('DRIVER='+driver+';SERVER='+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD='+ password)
cursor = cnxn.cursor()

sql  =""" select wrapperid from gearup_partnerzone where source ='CMS' and FileType <> 'Link' and status ='PUBLISHED'  and trim(title) ='10 Azure Stack HCI Facts' """
...
def upload_file( file_name,wrapperid):
    ...
    generator = block_blob_service.get_container_client("myvid")
